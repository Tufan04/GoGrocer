...
function AddSubCategory({ close }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setLoading(true)
            const response = await axiosInstance.post(addSubCategoryUrl, data)
            if (!response) {
                toast.error(response.data.message)
            }
            if (response.data.success) {
                dispatch(setSubCategories([response.data.data, ...subCategories]))
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <label htmlFor="subCategoryImage">
                            <div className={` bg-lime-600 text-white py-2 px-4 rounded-xl text-sm cursor-pointer`}>
                                {
