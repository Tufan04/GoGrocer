...
import { updateCategoryUrl } from "../config/ApiUrl.js"
import { errorToast } from "../utils/ToastHandler.js"
import { useDispatch, useSelector } from "react-redux"
import { setCategories } from "../redux/productSlice.js"

function EditCategoryItem({ categoryItemData, close }) {
    const [data, setData] = useState({
        id: categoryItemData._id,
        name: categoryItemData.name,
        image: categoryItemData.image
    })
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.success) {
                dispatch(setCategories(categories.map(category => category._id === data.id ? response.data.data : category)))
            ...
            <div className="max-w-md w-full bg-white p-4 rounded-xl">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <button className={`w-full mt-4 ${(data.name && data.image) ? "bg-black" : "bg-gray-400"} text-white py-3 px-4 rounded-lg text-sm font-semibold tracking-wide`}
                        disabled={!(data.name && data.image && setLoading)}>
