...
import { addProductUrl } from "../config/ApiUrl"
  ...
  return (
    <div>
      <div className="w-full bg-gray-100 rounded-xl px-4 py-3 flex items-center justify-between">
        <h4 className="text-xl font-semibold tracking-wide">Upload Product</h4>
      </div>

      <form onSubmit={submitHandler}>
        <div className="w-full mt-6 bg-blue-100 px-4 py-6 rounded-md">
          <div className="w-full sm:grid sm:grid-cols-2 gap-4 place-content-center">
            {/* Product Name */}
            <div className="w-full mb-2">
              <label className="block text-sm font-medium mb-1">Product Name</label>
              <input className="w-full px-3 py-2 outline-none rounded"
                type="text"
                name="name"
            ...
            <div className="w-full mb-2">
              ...
              {
                loading ? (
                  <div className="w-fit bg-gray-400 px-4 py-2 rounded font-semibold cursor-default" >Uploading ...</div>
                ) : (
                  <input className="w-full py-2 outline-none rounded"
                    type="file"
                    name="productImage"
            ...
            {/* images preview */}
            <div className={`w-full mb-2 col-span-2 flex flex-wrap gap-4 justify-center items-center ${data.image.length === 0 && "hidden"}`}>
                ...
                data?.image?.map((imge, idx) => (
                  <div key={idx} className="bg-white relative">
                    <img className="w-24 h-24 object-cover"
                      ...
                      alt={`Preview ${idx + 1}`}
                    />
                    <div className="text-red-500 hover:text-red-700 cursor-pointer absolute top-1 right-1"
                      onClick={() => {
                        setData((prev) => {
                          return {
                            ...prev,
            ...
            <div className="w-full mb-2">
              ...
              <select className="w-full px-3 py-2 outline-none rounded"
                name="category"
                value=""
                onChange={(e) => {
                ...
                <option value="" disabled>Select Category</option>
                {
                  categories.map((category, idx) => {
                    return (
                      <option key={"category" + idx} value={category.name}>{category.name}</option>
                    )
                  })
                }
              </select>
            ...
            <div className="w-full mb-2">
                        ...
                        <div className="text-red-500 hover:text-red-700 cursor-pointer"
                          onClick={() => {
                            setData((prev) => {
                              return {
                                ...prev,
                                subCategory: [...data.subCategory.filter(subCate => subCate.name !== option.name)]
                              }
                            })
                          }}>
                          <IoIosCloseCircle size={20} />
              ...
              <select className="w-full px-3 py-2 outline-none rounded"
                ...
                onChange={(e) => {
                  const value = e.target.value;
                  const subCategoryDetails = subCategories.find(subCategory => subCategory.name === value)
                  const isprevSelected = data.subCategory.find(subCat => subCat.name === value)
                  if (!isprevSelected) {
                    setData((prev) => {
                      return {
                        ...prev,
                        subCategory: [...data.subCategory, subCategoryDetails]
                      }
                    })
                  }
                }}>
                <option value="" disabled>Select Sub Category</option>
                {
                  subCategories.map((subCategory, idx) => {
                    return (
                      <option key={"subCategory" + idx} value={subCategory.name}>{subCategory.name}</option>
                    )
                  })
                }
              </select>
            ...
            <div className="w-full mb-2 col-span-2">
              ...
              <textarea className="w-full px-3 py-2 outline-none rounded"
                rows="3"
                name="description"
            ...
            {/* More Details */}
            {
              Object?.keys(data?.more_details)?.map((fieldName, index) => {
                return (
                  <div className="w-full mb-1 col-span-2" key={fieldName + index}>
            ...
            <div className="w-full col-span-2 flex justify-end mb-2">
              <div className="w-fit px-5 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded cursor-pointer"
            ...
            <div className="mb-4 flex items-center">
              <input className="mr-2 outline-none"
                type="checkbox"
                name="publish"
                id="publish"
              ...
              />
              <label htmlFor="publish" className="text-sm font-medium cursor-pointer">Publish Product</label>
            </div>

            {/* Submit Button */}
            <button className={`w-full mb-3 p-2 text-white rounded outline-none ${(data.name && data.image[0] && data.category[0] && data.subCategory[0] && data.unit && data.stock && data.price && data.description) ? "bg-blue-600 hover:bg-blue-700" : "bg-gray-400"}`}
